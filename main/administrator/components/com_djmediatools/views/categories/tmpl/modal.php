<?php 
/**
 * @version $Id: modal.php 22 2013-11-12 16:42:13Z szymon $
		...
		foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td align="center" class="center">
					<a href="#" onclick="if (window.parent) window.parent.<?php echo $function ?>('<?php echo $item->id ?>','<?php echo $item->image ? $item->image : 'administrator/components/com_djmediatools/assets/icon-album.png' ?>','<?php echo addslashes($this->escape($item->title)); ?>');">
					<?php if ($item->image) : ?>
				...
				<td align="center" valign="middle" class="center">
					<?php if($item->source=='component') { ?>
						<img src="components/com_djmediatools/assets/icon-48-slides.png" alt="<?php echo JText::_('COM_DJMEDIATOOLS_SUBMENU_SLIDES'); ?>" class="hasTip" title="<?php echo JText::_('COM_DJMEDIATOOLS_SUBMENU_SLIDES'); ?>" width="32" /></a>
					<?php } else { ?>
				...
				<td>
					...
					<a href="#" class="pointer" onclick="if (window.parent) window.parent.<?php echo $function ?>('<?php echo $item->id; ?>','<?php echo $item->image ? $item->image : 'administrator/components/com_djmediatools/assets/icon-album.png' ?>','<?php echo addslashes($this->escape($item->title)); ?>');">
