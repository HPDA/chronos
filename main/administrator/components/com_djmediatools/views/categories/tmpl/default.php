<?php 
/**
 * @version $Id: default.php 18 2013-10-01 15:04:53Z szymon $
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td align="center" valign="middle" class="center">
					<?php if($item->source=='component') { ?>
						<a href="<?php echo JRoute::_('index.php?option=com_djmediatools&view=items&filter_category='. $item->id); ?>"><img src="components/com_djmediatools/assets/icon-48-slides.png" alt="<?php echo JText::_('COM_DJMEDIATOOLS_SUBMENU_SLIDES'); ?>" class="hasTip" title="<?php echo JText::_('COM_DJMEDIATOOLS_SUBMENU_SLIDES'); ?>" /></a>
					<?php } else { ?>
				...
				<td class="order" nowrap="nowrap">
					<?php if ($canChange) : ?>
						<?php if ($saveOrder) :?>
							<?php if ($listDirn == 'asc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, ($this->ordering[$item->key]->parent_id == @$this->ordering[$item->key-1]->parent_id),'categories.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $n, ($this->ordering[$item->key]->parent_id == @$this->ordering[$item->key+1]->parent_id), 'categories.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php elseif ($listDirn == 'desc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, ($this->ordering[$item->key]->parent_id == @$this->ordering[$item->key-1]->parent_id),'categories.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $n, ($this->ordering[$item->key]->parent_id == @$this->ordering[$item->key+1]->parent_id), 'categories.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php endif; ?>
						<?php endif; ?>
